package cm.softinovplus.mobilebiller.db;
...
public class SMSDataSource {
	...
	public boolean updateSMS(long id, ContentValues values){
		String [] params = {"" + id};
		   int a  = database.update(MySQLiteHelper.TABLE_SMS, values, "" +MySQLiteHelper.COLUMN_ID
			        + "=?"  , params);
		   
		   //Toast.makeText(mContext, "updated: " + a + "_" + values, Toast.LENGTH_LONG).show();
		   return (a==1);
	...
	public SMS getSMSById(long id){
		String args[] = {""+id};
		Cursor cursor = database.query(MySQLiteHelper.TABLE_SMS, allColumns, "_id=?" , args, null, null, null);
